...
impl Config {
	pub fn build(args: &[String]) -> Result<Config, &'static str> {
		...
		let pretty = args.get(3).unwrap_or(&"true".to_string()).clone().parse::<bool>().map_err(|_| "3rd option should be true or false")?;
		
		if !Path::new(&input_folder).is_dir() || !Path::new(&output_folder).is_dir() {
			if Path::new(&input_folder).is_dir() && !Path::new(&output_folder).try_exists().map_err(|_| "Could not check if output location exists!")? {
